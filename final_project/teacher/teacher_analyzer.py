...
# ══════════════════════════════════════════════════════════════
#  DEFAULT MODEL/VIDEO FOLDER
#  Anchored to this script's own directory, so best_model.pth /
#  enthusiasm_lstm.pt / feature_scaler.pkl are found no matter what
#  the current working directory is when the script is launched.
# ══════════════════════════════════════════════════════════════
DEFAULT_TEACHER_FOLDER = os.path.dirname(os.path.abspath(__file__))
# ══════════════════════════════════════════════════════════════
...
def main():
    ...
    print("VIDEO DETAILS")
    print("=" * 70)
    print(f"  Name        : {Path(video_path).name}")
    print(f"  Path        : {video_path}")
    print(f"  Resolution  : {width} x {height}")
    print(f"  FPS         : {fps_src:.2f}")
    print(f"  Frame count : {total_frames_cap}")
    print(f"  Duration    : {true_duration:.1f} sec  ({true_duration/60:.2f} min)")
    print("=" * 70, flush=True)

    # Writer is created lazily on the first written frame so its size
    # matches the actual overlay/display frame (handles downscaling).
    writer = None
    _writer_fourcc = cv2.VideoWriter_fourcc(*"mp4v")
    ...
    # Speed: downscale large frames before analysis (accuracy preserved).
    try:
        _tmax_w = int(os.environ.get('ANALYSIS_MAX_WIDTH', '960'))
    except Exception:
        _tmax_w = 960
    while True:
        if not paused:
            ...
            if _tmax_w and frame.shape[1] > _tmax_w:
                _tsc = _tmax_w / float(frame.shape[1])
                frame = cv2.resize(frame, (_tmax_w, int(frame.shape[0] * _tsc)),
            ...
            if writer is None and display is not None:
                _dh, _dw = display.shape[:2]
                writer = cv2.VideoWriter(out_video, _writer_fourcc, fps_src, (_dw, _dh))
